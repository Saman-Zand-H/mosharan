...
class RawEventQuerySet(CompanyScopedQuerySet):
    company_user_lookup = "gateway__company__user_id"


class ParameterReadingQuerySet(CompanyScopedQuerySet):
    company_user_lookup = "raw_event__gateway__company__user_id"
...
class VisualizationTab(TimestampedMixin):
    """An ordered dashboard group shared by one device type."""
    ...
    class Meta:
        verbose_name = _("Visualization Tab")
        verbose_name_plural = _("Visualization Tabs")
        constraints = [
...
class Visualization(TimestampedMixin):
    """A database-defined chart within a visualization tab."""
    ...
    # Null means the canonical observed_at timestamp axis.
    x_axis = models.ForeignKey(
        ...
        verbose_name=_("X Axis Parameter"),
    )
    y_axis = models.ForeignKey(
        ParameterDefinition,
        on_delete=models.PROTECT,
        related_name="y_axis_visualizations",
        verbose_name=_("Y Axis Parameter"),
    )
    sort_order = models.PositiveIntegerField(default=0, verbose_name=_("Sort Order"))
    is_active = models.BooleanField(default=True, verbose_name=_("Is Active"))
    ...
    def clean(self) -> None:
        super().clean()
        errors = {}
        tab = self.tab if self.tab_id is not None else None
        x_axis = self.x_axis if self.x_axis_id is not None else None
        y_axis = self.y_axis if self.y_axis_id is not None else None
        chartable_types = {
            ParameterDefinition.ValueType.INTEGER,
            ParameterDefinition.ValueType.DATETIME,
        }
        if tab is not None:
            assigned = DeviceTypeParameter.objects.filter(
                device_type_id=tab.device_type_id,
            )
            if y_axis is not None and not assigned.filter(parameter=y_axis).exists():
                errors["y_axis"] = _(
                    "Y-axis parameter is not assigned to the tab device type."
                )
            if x_axis is not None and not assigned.filter(parameter=x_axis).exists():
                errors["x_axis"] = _(
                    "X-axis parameter is not assigned to the tab device type."
                )
        if y_axis is not None and y_axis.value_type not in chartable_types:
            errors["y_axis"] = _(
                "Y-axis parameter must be integer or datetime for a chart."
            )
        if x_axis is not None and x_axis.value_type not in chartable_types:
            errors["x_axis"] = _("X-axis parameter must be integer or datetime.")
        if x_axis is not None and y_axis is not None and x_axis.pk == y_axis.pk:
            errors["x_axis"] = _("X-axis and Y-axis parameters must differ.")
        if errors:
...
class PayloadSchema(TimestampedMixin):
    ...
    def clean(self) -> None:
        super().clean()
        if self.pk is None or self.expected_length is None:
            return
        if PayloadField.objects.filter(
            payload_schema=self,
            end_byte__gt=self.expected_length,
        ).exists():
            raise ValidationError(
                {
                    "expected_length": _(
                        "Expected length is shorter than a payload field."
                    )
                }
            )
...
class PayloadField(TimestampedMixin):
    ...
    def clean(self) -> None:
        ...
        if (
            payload_schema is not None
            and PayloadField.objects.filter(
                payload_schema=payload_schema,
                start_byte__lt=self.end_byte,
                end_byte__gt=self.start_byte,
            )
            .exclude(pk=self.pk)
            .exists()
        ):
            errors["start_byte"] = _("Payload fields cannot overlap.")
        if errors:
            raise ValidationError(errors)
...
class ProjectionRule(TimestampedMixin):
    ...
    def clean(self) -> None:
        ...
        if source_field is not None and parameter is not None:
            compatible_types: dict[str, set[str]] = {
                PayloadField.WireCodec.INTEGER.value: {
                    ParameterDefinition.ValueType.INTEGER.value,
                    ParameterDefinition.ValueType.DATETIME.value,
                },
                PayloadField.WireCodec.UTF8.value: {
                    ParameterDefinition.ValueType.STRING.value,
                },
                PayloadField.WireCodec.BOOLEAN.value: {
                    ParameterDefinition.ValueType.BOOLEAN.value,
                },
            }
            if parameter.value_type not in compatible_types.get(
                source_field.wire_codec,
                set(),
            ):
                errors["parameter"] = _(
                    "Parameter type is incompatible with source field codec."
                )
            if (
                source_field.role == PayloadField.Role.TIMESTAMP
                and parameter.value_type != ParameterDefinition.ValueType.DATETIME
            ):
                errors["parameter"] = _(
                    "Timestamp fields must project to datetime parameters."
                )
        if parameter is not None:
            config = self.conversion_config
            if not isinstance(config, dict):
                errors["conversion_config"] = _("Conversion config must be an object.")
            elif parameter.value_type == ParameterDefinition.ValueType.DATETIME:
                if set(config) - {"timestamp_unit"} or config.get(
                    "timestamp_unit",
                    "seconds",
                ) not in {"seconds", "milliseconds"}:
                    errors["conversion_config"] = _(
                        "Datetime conversion supports seconds or milliseconds only."
                    )
            elif config:
                errors["conversion_config"] = _(
                    "Conversion config is supported for datetime parameters only."
                )
            if self.source_kind == self.SourceKind.CONSTANT:
                value = self.constant_value
                expected_types: dict[str, type[int] | type[str] | type[bool]] = {
                    ParameterDefinition.ValueType.INTEGER.value: int,
                    ParameterDefinition.ValueType.DATETIME.value: int,
                    ParameterDefinition.ValueType.STRING.value: str,
                    ParameterDefinition.ValueType.BOOLEAN.value: bool,
                }
                expected_type = expected_types.get(parameter.value_type)
                if expected_type is not None and type(value) is not expected_type:
                    errors["constant_value"] = _(
                        "Constant value must match parameter type."
                    )
        if errors:
            raise ValidationError(errors)
...
class RawEvent(models.Model):
    ...
    objects = RawEventQuerySet.as_manager()
...
class ParameterReading(models.Model):
    ...
    objects = ParameterReadingQuerySet.as_manager()
